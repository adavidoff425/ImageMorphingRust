...
//  assert_eq!(env::args().count(), 3);
  ...
  let (vertices, indices) = {
    ...
    let index_buf = glium::IndexBuffer::new(&display_src, index::PrimitiveType::TrianglesList, &data).unwrap();
  ...
  let (vertices, indices) = {
    ...
    let index_buf = glium::IndexBuffer::new(&display_dst, index::PrimitiveType::TrianglesList, &data).unwrap();
    (vertex_buf, index_buf)
  };
